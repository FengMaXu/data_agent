...
export function canonicalLocalTools(): CanonicalTool[] {
  return [
    ...
    { name: "read_file", identity: "read_file", origin: "local", description: "Read a workspace file, optionally selecting one-based inclusive lines. Results are capped at 50 KiB.", parameters: Type.Object({ path: Type.String(), startLine: Type.Optional(Type.Integer({ minimum: 1 })), endLine: Type.Optional(Type.Integer({ minimum: 1 })) }) },
    ...
    { name: "search_knowledge", identity: "search_knowledge", origin: "local", description: "Search the Markdown knowledge base and return readable matching snippets with locations.", parameters: Type.Object({ query: Type.String({ minLength: 1 }) }) },
    { name: "read_knowledge", identity: "read_knowledge", origin: "local", description: "Read a knowledge document, optionally selecting one-based inclusive lines. Results are capped at 50 KiB.", parameters: Type.Object({ path: Type.String({ minLength: 1 }), startLine: Type.Optional(Type.Integer({ minimum: 1 })), endLine: Type.Optional(Type.Integer({ minimum: 1 })) }) },
